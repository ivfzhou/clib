...
const static char _operators[] = "+-*/()";

static _Bool _isOperator(char c) {
    int i = 0;
    char o;
    while ((o = _operators[i++])) if (c == o) return 1;
    return 0;
}
...
static long double _calculate(long double x, char optr, long double y) {
    switch (optr) {
        case '+':
            return x + y;
        case '-':
...
long double expression_calculate(const char *expr) {
    Stack *operator = stack_alloc(sizeof(char));
    Stack *operand = stack_alloc(sizeof(long double));
        ...
        if (!_isOperator(c)) {
            ...
            switch (isPrecede(optr, c)) {
                case 1:
                    ...
                    stack_pop(operand, &x);
                    stack_pop(operand, &y);
                    long double res = _calculate(y, optr, x);
